...
const STEPS = [
  { label: "Informations", index: 0 },
  { label: "Ingrédients", index: 1 },
  { label: "Récapitulatif", index: 2 },
]
...
export default function NewDishPage() {
  ...
  const derivedAllergens = useMemo(() => {
    ...
    return ALL_ALLERGENS.filter((a) => set.has(a))
  }, [selectedIngredients])

  const derivedDiets = useMemo(() => {
    if (selectedIngredients.length === 0) return []
    return ALL_DIETS.filter((d) =>
      selectedIngredients.every(({ ingredient }) => ingredient.diets.includes(d))
    )
  }, [selectedIngredients])

  const canAdvanceStep1 = name.trim().length > 0 && description.trim().length > 0 && image.length > 0
  const canAdvanceStep2 = selectedIngredients.length > 0
  ...
  return (
    <div className="flex flex-1 flex-col gap-0">
      ...
      <div className="flex items-center gap-2 pb-6">
        ...
        <h1 className="text-lg font-bold">Nouveau plat</h1>
      </div>
        ...
        <div className="flex flex-1 flex-col gap-8">
          ...
          <section className="flex flex-col gap-3">
            ...
            <label className="group relative h-48 w-full cursor-pointer overflow-hidden rounded-xl bg-muted">
              {image ? (
                <Image src={image} alt={name || "Plat"} fill className="object-cover" sizes="900px" />
              ) : (
          ...
          <section className="flex flex-col gap-4">
            ...
            <FieldGroup>
              <Field>
                <FieldLabel htmlFor="dish-name">Nom du plat</FieldLabel>
                <Input
                  id="dish-name"
                  value={name}
      ...
      {step === 1 && (
        <div className="flex flex-1 flex-col gap-6">
          ...
          <div className="pb-6">
            <Button className="w-full" disabled={!canAdvanceStep2} onClick={() => setStep(2)}>
              Continuer
              <ChevronRight data-icon="inline-end" />
            </Button>
          </div>
      ...
      {step === 2 && (
        <div className="flex flex-1 flex-col gap-6">
          ...
          <Card className="overflow-hidden">
            {image && (
              <div className="relative h-48 w-full">
                <Image src={image} alt={name} fill className="object-cover" sizes="900px" />
              </div>
            )}
            {!image && (
            ...
            <CardHeader>
              <CardTitle>{name || "Sans nom"}</CardTitle>
          ...
          <div className="flex flex-col gap-3 pb-6">
            <Button
              className="w-full"
              onClick={() => router.push(`/restaurant/${slug}/dashboard/manage-menu`)}
            >
